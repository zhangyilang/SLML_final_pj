...
class SkipLSTMCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            # Apply update gate
            new_c = tf.sigmoid(f + self._forget_bias) * c_prev + update_gate * c_prev * tf.sigmoid(i) * self._activation(j)
            new_h = ((1. - update_gate) * tf.sigmoid(o) + update_gate * tf.sigmoid(o_prev)) * tf.tanh(new_c)
